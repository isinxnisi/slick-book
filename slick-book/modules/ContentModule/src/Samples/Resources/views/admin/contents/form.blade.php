...
@section('content')
    @php
        ...
        $initialType = old('content_type', $type);
        $initialKind = old('content_kind', $kind);
    ...
    @if ($errors->count())
        <div class="w-100">
            <p class="text-danger">入力エラーがあります</p>
            @php
                print_r($errors->all());
            @endphp
        </div>
    ...
    <div class="p-4">
        ...
        <form id="content-form" method="POST"
            action="{{ $isEdit ? route('admin.contents.update', $entity->getId()) : route('admin.contents.store') }}">
            @csrf
            @if ($isEdit)
                @method('PUT')
            @endif
            ...
            {{-- ←ここから Strategy 切り替えフォームを表示 --}}
            <div id="form-fields">
                {!! $formHtml !!}
            </div>

            <button type="submit" class="mt-4 bg-green-500 …">
                {{ $isEdit ? '更新' : '保存' }}
            </button>
        </form>
        ...
        @if ($isEdit)
            <div class="mt-6 space-x-2">
                ...
                @if ($entity->getStatus() === 'draft')
                    <form method="POST" action="{{ route('admin.contents.to-review', $entity->getId()) }}" class="inline">
                        @csrf
                ...
                @if ($entity->getStatus() === 'review')
                    <form method="POST" action="{{ route('admin.contents.publish', $entity->getId()) }}" class="inline">
                        @csrf
                ...
                @if ($entity->getStatus() === 'published')
                    <form method="POST" action="{{ route('admin.contents.archive', $entity->getId()) }}" class="inline">
                        @csrf
                        <button type="submit" class="bg-gray-600 text-white px-3 py-1 rounded">
                            アーカイブ
                        </button>
                    </form>
                @endif
